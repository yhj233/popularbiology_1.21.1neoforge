...
public class LizimantouPet extends BasePet {
   public LizimantouPet(EntityType<? extends BasePet> type, Level level) {
      super(type, level, "lizimantou");
   }
   ...
   public PetSoundManager getSoundManager() {
      return (new PetSoundManager.Builder()).tameSound((SoundEvent)InitSound.LIZIMANTOU_TAME_SOUND.get()).hurtSound((SoundEvent)InitSound.LIZIMANTOU_GETHURT_SOUND.get()).deathSound((SoundEvent)InitSound.LIZIMANTOU_DEATH_SOUND.get()).build();
   }
   @Override
