...
public class ShisaPet extends BasePet {
   public ShisaPet(EntityType<? extends BasePet> type, Level level) {
      super(type, level, "shisa");
   }
   ...
   public PetSoundManager getSoundManager() {
      return (new PetSoundManager.Builder()).tameSound((SoundEvent)InitSound.SHISA_TAME_SOUND.get()).hurtSound((SoundEvent)InitSound.SHISA_GETHURT_SOUND.get()).deathSound((SoundEvent)InitSound.SHISA_DEATH_SOUND.get()).build();
   }
   @Override
