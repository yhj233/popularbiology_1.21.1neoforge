...
public class WalkToPlantCropTask extends Behavior<BasePet> {
   private static final Map<MemoryModuleType<?>, MemoryStatus> REQUIRED_MEMORIES;
   private final float speed;

   public WalkToPlantCropTask(float speed) {
      super(REQUIRED_MEMORIES, 15);
      this.speed = speed;
   }

   protected boolean checkExtraStartConditions(ServerLevel world, BasePet pet) {
      return pet.getState() == PetState.WORK && pet.getJobId() == 1 && !pet.getBrain().getMemory((MemoryModuleType)InitMemory.HARVEST_POS.get()).isPresent() && Utils.getSeed(pet) != null && pet.getBrain().getMemory((MemoryModuleType)InitMemory.PLANT_POS.get()).isPresent() && pet.getAction() == 0;
   }

   protected void start(ServerLevel world, BasePet pet, long time) {
      BehaviorUtils.setWalkAndLookTargetMemories(pet, (BlockPos)pet.getBrain().getMemory((MemoryModuleType)InitMemory.PLANT_POS.get()).get(), this.speed, 0);
   }
