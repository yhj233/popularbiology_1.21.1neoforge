...
public class WalkToContainerTask extends Behavior<BasePet> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel world, BasePet pet) {
      return !pet.getBrain().getMemory((MemoryModuleType)InitMemory.PLANT_POS.get()).isPresent() && !pet.getBrain().getMemory((MemoryModuleType)InitMemory.HARVEST_POS.get()).isPresent() && pet.getState() == PetState.WORK && Utils.isFitCondition(pet.getInventory(), InitTag.ENTITY_DELIVER_ITEM) != null && pet.getAction() == 0;
   }

   protected void start(ServerLevel world, BasePet pet, long time) {
      BehaviorUtils.setWalkAndLookTargetMemories(pet, (BlockPos)pet.getBrain().getMemory((MemoryModuleType)InitMemory.CONTAINER_POS.get()).get(), this.speed, 3);
   }
